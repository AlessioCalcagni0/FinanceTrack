document.addEventListener('DOMContentLoaded', () => {
    ...
    async function caricaDati(periodo = "anno") {
        const res = await fetch(`http://localhost:5432/api.php?path=api/spese&periodo=${periodo}`);
    ...
    confirmButton.addEventListener("click", async () => {
        ...
        try {
            const res = await fetch("http://localhost:5432/api.php?path=addCategory", {
                method: "POST",
...
async function loadCategories() {
    try {
        ...
        const res = await fetch("http://localhost:5432/api.php?path=categories");
