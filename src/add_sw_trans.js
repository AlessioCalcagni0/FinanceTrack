document.addEventListener("DOMContentLoaded", async () => {
    async function loadWallets(userId) {
        ...
        try {
            const url = `http://${API_HOST}:8000/api.php?path=fetchSW&shared_wallet=${encodeURIComponent(uid)}`;
            ...
            // Lo shared wallet unico
            const wallet = wallets[0];

            // Costruisco array partecipanti
            const otherNames = [
                wallet.partecipant_name_surname1,
                wallet.partecipant_name_surname2,
                wallet.partecipant_name_surname3
            ].map(v => (v ?? '').trim()).filter(v => v.length > 0);



            const participants = ["Me"];
            for (let i = 1; i <= wallet.partecipant_num; i++) {
                const name = wallet[`partecipant_name_surname${i}`];
                if (name && name.trim() !== "") {
                    participants.push(name.trim());
                }
            }

            // Sezione payer (checkbox a sinistra)
            ...
            participants.forEach((p, idx) => {
                ...
                div.innerHTML = `
            <input type="radio" id="payer_${idx}" name="payer" value="${p}">
            <label for="payer_${idx}">${p}</label>
        `;
                payerSection.appendChild(div);

            });

            // Sezione custom split (input amount a destra)
            ...
            participants.forEach((p, idx) => {
                ...
                div.innerHTML = `
            <label>${p}</label>
            <input type="number" min="0" max="9999" placeholder="€" name="amount_${idx}" value="0.00">
        `;
                splitSection.appendChild(div);
            });
    ...
    function resetAllFields() {
        // Reset di tutti gli input, textarea e select
        ...
        // Svuoto le sezioni dinamiche dei partecipanti
        const payerSection = document.getElementById("payerSection");
        const splitSection = document.getElementById("splitSection");
        if (payerSection) payerSection.innerHTML = "";
    ...
    const totalInput = document.querySelector(".number_input"); // totale spesa
    const splitSection = document.getElementById("splitSection");

    function updateSplitValues() {
        const total = parseFloat(totalInput.value) || 0;
        const participants = splitSection.querySelectorAll(".participant input[type='number']");
        if (participants.length === 0) return;

        const perHead = (total / participants.length).toFixed(2);
        console.log("number ", participants.length);
        participants.forEach(input => {
            input.value = perHead;
        });
    }

    // Aggiorna quando digiti o cambi il totale
    totalInput.addEventListener("input", updateSplitValues);

    // Aggiorna anche al caricamento iniziale
    ...
    async function confirmAction() {
        ...
        // 🔹 Controllo payer selezionato
        const payer = document.querySelector("input[name='payer']:checked");
        if (!payer) {
            showPopup("Select who made the transaction.");
            return;
        }

        // 🔹 Recupero i valori dei partecipanti
        const participantsInputs = document.querySelectorAll("#splitSection .participant input[type='number']");
        const participantsLabels = document.querySelectorAll("#splitSection .participant label");
    ...
    function showCancelPopup(e) {
        ...
        const clearAllFieldsAndClose = () => {
            resetAllFields();
            closeAll();
            window.location.href = "/sharedWallet.php";  // ✅ redirect
        };
